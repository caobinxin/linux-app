...
/* 定义每个串口结构体变量 */
	 ...
	 uint8_t uart1_rx_flag = 0;                 // 接收1帧数据标识
#endif
	 ...
	 uint8_t uart2_rx_flag = 0;                 // 接收1帧数据标识
#endif
...
static void InitHardUart(void)
{
	...
	*/
	...
	USART_ITConfig(USART1, USART_IT_IDLE, ENABLE);	/* 使能空闲中断-- 接收到1帧数据*/
	/*
	...
	*/
	...
	USART_ITConfig(USART2, USART_IT_IDLE, ENABLE);	/* 使能空闲中断-- 接收到1帧数据*/
	/*
		USART_ITConfig(USART1, USART_IT_TXE, ENABLE);
		注意: 不要在此处打开发送中断
		发送中断使能在SendUart()函数打开
	...
	*/
	...
	USART_ITConfig(USART3, USART_IT_IDLE, ENABLE);	/* 使能空闲中断-- 接收到1帧数据*/
	/*
		USART_ITConfig(USART1, USART_IT_TXE, ENABLE);
		注意: 不要在此处打开发送中断
		发送中断使能在SendUart()函数打开
...
/*
	...
	{USART_ClearFlag(_pUart->uart,USART_FLAG_ORE);USART_ReceiveData(_pUart->uart);
	quanju=2;
...
//		USART_ClearITPendingBit(_pUart->uart,USART_IT_RXNE); 
		...
		if (USART_GetITStatus(USART1, USART_IT_IDLE) != RESET)
		{
	        ...
	        clear = USART1->DR;  // 读数据态寄存器(先读取状态，再读取数据寄存器，目的是清除IDLE标志)
	        uart1_rx_flag = 1;
		}
		/* 处理串口2接收1帧数据中断  */
		if (USART_GetITStatus(USART2, USART_IT_IDLE) != RESET)
		{
			clear = USART2->SR;  // 读取状态寄存器
	        clear = USART2->DR;  // 读数据态寄存器(先读取状态，再读取数据寄存器，目的是清除IDLE标志)
	        uart2_rx_flag = 1;
		}
		/* 处理串口3接收1帧数据中断  */
		if (USART_GetITStatus(USART3, USART_IT_IDLE) != RESET)
		{
			clear = USART3->SR;  // 读取状态寄存器
	        clear = USART3->DR;  // 读数据态寄存器(先读取状态，再读取数据寄存器，目的是清除IDLE标志)
	        //uart3_rx_flag = 1;
			g_imu_time = get_system_time();  // 记录imu时间戳
	       
		    xHigherPriorityTaskWoken = pdFALSE;
	        xSemaphoreGiveFromISR(xSemaphore_imu_h,&xHigherPriorityTaskWoken);
